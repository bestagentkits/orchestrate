...
The orchestration scope names Claude Code as a runtime to support. This
repository contains no probe evidence for it, so nothing about its headless
surface, controls, capture, or resume behavior is known here.

Writing a capability map from memory would be exactly the failure this skill
forbids: a copied catalog presented as live evidence.

## Probe focus

A future probe must establish, from the installed version's own help:

- the non-interactive or headless entry surface;
- working-directory control;
- model discovery and selection;
- permission and approval controls, and whether headless mode auto-approves;
- tool gating, including whether write, shell and network access can be denied
  independently;
- structured output or event capture, and where the final result lands;
- session identity, continuation and fork behavior;
...
5. Record the profile with `role: runtime`, writing every unproven control
   `unverified`. An unproven control is never written as available.
